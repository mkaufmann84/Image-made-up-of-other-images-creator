...
resize_scale=4/7  #makes images 16 by 16
os.chdir('mnist_imgs')
images=[]

for file in os.listdir():
    filler_img = cv2.imread(file)
    filler_img=cv2.cvtColor(filler_img, cv2.COLOR_BGR2RGB)
    filler_img = resize(filler_img,resize_scale)
    images.append(filler_img)
#cv2 uses Blue Green Red instead of normal RGB



resize_filler = resize(filler_img,1)  #smaller value means more filler image, but they will be of less quality
...
def create_img(img,step_r,step_c,alpha,images):
    counter =0
    l_images=len(images)
    for row in range(0,img.shape[0],step_r): #row is really the height
        for col in range(0,img.shape[1],step_c):   #nump array has img as height, width, channels
            ...
            resize_filler= images[counter%l_images]
            counter+=1
...
edit_img = create_img(img,step_r,step_c,alpha,images)

Image.fromarray(edit_img).show()  #shows finished img

os.chdir("/Users/max/Desktop")   
cv2.imwrite('obama_mnist.jpg',cv2.cvtColor(edit_img, cv2.COLOR_RGB2BGR)) #saves photo.
